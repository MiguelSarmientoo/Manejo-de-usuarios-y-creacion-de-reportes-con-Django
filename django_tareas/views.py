...
def descargarReporteUsuarios(request):
    ...
    buffer = BytesIO()  # create an in-memory PDF buffer

    archivoPdf = canvas.Canvas(buffer, pagesize=A4)
    ...
    archivoPdf.setFont('Helvetica-Bold',12)
    archivoPdf.drawString(40,680, 'Fecha de creación')
    archivoPdf.drawString(40,665, 'Cantidad usuarios:')
    archivoPdf.drawString(40,650, 'Usuario que generó reporte')
    archivoPdf.drawString(40,635, 'Tipo de usuario')
    ...
    for usuario in usuariosTotales:
        ...
        archivoPdf.drawString(lista_x[0] + 155, lista_y[1]-15, f'Nombre Completo: {usuario.first_name}, {usuario.last_name}')

        if datosUsuario.objects.filter(user=usuario).exists():
            fechaIngreso = datosUsuario.objects.get(user=usuario).fechaIngreso
            archivoPdf.drawString(lista_x[0] + 325, lista_y[1]-35, f'Fecha de creación: {fechaIngreso}')  
        else:
            archivoPdf.drawString(lista_x[0] + 325, lista_y[1]-35, f'Fecha de creación: -')  
            
        archivoPdf.drawString(lista_x[0] + 155, lista_y[1]-55, f'Tipo de Usuario: {usuario.datosusuario.tipoUsuario}')
